...
{
    ScamValue expr = readEval("(max 123 -123)");
    expectInteger(expr, 123, "123", true);
}

TEST_F(PreludeTest, MaxRealTest)
{
    ScamValue expr = readEval("(max 42.01 17.5)");
...
{
    ScamValue expr = readEval("(max -5 -9.999)");
    expectInteger(expr, -5, "-5", true);
}

TEST_F(PreludeTest, MinIntegerTest)
{
    ScamValue expr = readEval("(min 123 -123)");
    expectInteger(expr, -123, "-123", true);
}

TEST_F(PreludeTest, MinRealTest)
{
    ScamValue expr = readEval("(min 42.01 17.5)");
...
{
    ScamValue expr = readEval("(min -5 -9.999)");
...
{
    ScamValue expr = readEval("(map integer? (list 1 0.123 \"xx\"))");
    expectList(expr, "(#t #f #f)", 3);
}

TEST_F(PreludeTest, ReduceEmptyList)
{
    ScamValue expr = readEval("(reduce (lambda (a b) (+ a b)) 0 '())");
    expectInteger(expr, 0, "0", true);
}

TEST_F(PreludeTest, ReduceSingleton)
{
    ScamValue expr = readEval("(reduce (lambda (a b) (+ a b)) 0 '(5))");
    expectInteger(expr, 5, "5", true);
}

TEST_F(PreludeTest, ReduceMany)
{
    ScamValue expr = readEval("(reduce (lambda (a b) (+ a b)) 0 '(1 2 3 4 5))");
...
{
    ScamValue expr =
        readEval("(reduce (lambda (a b) (+ a b)) -10 '(1 2 3 4 5))");
    expectInteger(expr, 5, "5", true);
}

TEST_F(PreludeTest, ReduceListOps)
{
    ScamValue expr = readEvalFile("scripts/prelude/reducelist1.scm");
    expectList(expr, "(#f 1 3 2)", 4);
}

TEST_F(PreludeTest, ReduceListOps2)
{
    ScamValue expr = readEvalFile("scripts/prelude/reducelist2.scm");
    expectList(expr, "((#t) (1 3) (2))", 3);
}

TEST_F(PreludeTest, FilterNil)
{
    ScamValue expr = readEval("(filter even? '())");
    expectNull(expr);
}

TEST_F(PreludeTest, FilterList)
{
    ScamValue expr = readEval("(filter even? '(1 2 3 4 5))");
...
TEST_F(PreludeTest, RequireTest)
{
    ...
    ScamValue expr = readEval("(require #f)");
...
{
    ScamValue expr = readEval("(define x (list 1 2 3)) (exclude x x)");
    expectNull(expr);
}

TEST_F(PreludeTest, ExcludePartial)
{
    ScamValue expr = readEval("(define x (list 1 2 3)) (exclude (cdr x) x)");
    expectList(expr, "(1)", 1);
}

TEST_F(PreludeTest, ExcludeNonOverlapping)
{
    ScamValue expr = readEval("(exclude (list 1 2) (list 3 4))");
    expectList(expr, "(3 4)", 2);
}

TEST_F(PreludeTest, LengthVector)
{
    ScamValue expr = readEval("(old-length #(1 2 3))");
    expectInteger(expr, 3, "3", true);
}

TEST_F(PreludeTest, LengthDict)
{
    ScamValue expr = readEval("(old-length { :a 44 :b \"cat\" })");
    expectInteger(expr, 2, "2", true);
}

TEST_F(PreludeTest, LengthBadType)
{
    ScamValue expr = readEval("(old-length :abc)");
    expectError(expr);
}

TEST_F(PreludeTest, NthList)
{
    ScamValue expr = readEval("(nth 0 '(a b c))");
    expectSymbol(expr, "a");
}

TEST_F(PreludeTest, NthListOutOfBounds)
{
    ScamValue expr = readEval("(nth 4 '(a b c))");
    expectError(expr);
}

TEST_F(PreludeTest, NthVector)
{
    ScamValue expr = readEval("(nth 0 #(1 2 3))");
    expectInteger(expr, 1, "1", true);
}

TEST_F(PreludeTest, NthVectorOutOfBounds)
{
    ScamValue expr = readEval("(nth -50 #(1 2 3))");
    expectError(expr);
}

TEST_F(PreludeTest, NthNotIndexible)
{
    ScamValue expr = readEval("(nth 0 { :a 333.333 })");
    expectError(expr);
}

TEST_F(PreludeTest, CrossNilWithNil)
{
    ScamValue expr = readEval("(cross cons '() '())");
    expectNull(expr);
}

TEST_F(PreludeTest, CrossNilWithList)
{
    ScamValue expr = readEval("(cross cons '() '((1) (2)))");
    expectNull(expr);
}

TEST_F(PreludeTest, CrossListWithNil)
{
    ScamValue expr = readEval("(cross (lambda (a b) (+ a b)) '(1 2 3) '())");
    expectNull(expr);
}

TEST_F(PreludeTest, CrossListWithList)
{
    ScamValue expr = readEvalFile("scripts/prelude/crosstest1.scm");
    expectList(expr, "(2 3 4 3 4 5 4 5 6)", 9);
}

TEST_F(PreludeTest, PowerSetNil)
{
    ScamValue expr = readEval("(power-set '())");
    expectList(expr, "(())", 1);
}

TEST_F(PreludeTest, PowerSetSingleton)
{
    ScamValue expr = readEval("(power-set '(1))");
    expectList(expr, "(() (1))", 2);
}

TEST_F(PreludeTest, PowerSetList)
{
    ScamValue expr = readEvalFile("scripts/prelude/powerset.scm");
...
{
    ScamValue expr = readEval("(some-of '())");
    expectError(expr, "No more choices", false);
}

TEST_F(PreludeTest, SomeOfOne)
{
    ScamValue expr = readEval("(some-of '(1))");
    expectList(expr, "(1)", 1);
...
{
    ScamValue expr = readEval("(cond ())");
    expectNull(expr);
}

TEST_F(PreludeTest, CondOneTrueClause)
{
    ScamValue expr = readEvalFile("scripts/cond/onetrue.scm");
    expectString(expr, "\"One\"");
}

TEST_F(PreludeTest, CondOneFalseClause)
{
    ScamValue expr = readEvalFile("scripts/cond/onefalse.scm");
    expectNull(expr);
}

TEST_F(PreludeTest, CondManyClauses)
{
    ScamValue expr = readEvalFile("scripts/cond/many.scm");
    expectString(expr, "\"Last\"");
}

TEST_F(PreludeTest, CondElse)
{
    ScamValue expr = readEvalFile("scripts/cond/else.scm");
    expectString(expr, "\"Else Evaluated\"");
}

TEST_F(PreludeTest, CondManyForms)
{
    ScamValue expr = readEval("(cond ((#t 1 2 3)))");
    expectInteger(expr, 3, "3", true);
}

TEST_F(PreludeTest, CondNoForms)
{
    ScamValue expr = readEval("(cond ((3)))");
    expectInteger(expr, 3, "3", true);
}

TEST_F(PreludeTest, CondArrowForm)
{
    (void) readEval("(define inc (lambda (x) (+ x 1)))");
    ScamValue expr = readEval("(cond ((1 => inc)))");
    expectInteger(expr, 2, "2", true);
}
